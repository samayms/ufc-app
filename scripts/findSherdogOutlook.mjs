/**
 * Finds the current card's Sherdog fight-outlook (preview) article through
 * Sherdog's official news RSS feed. It prints the env assignment but never
 * edits .env.
 *
 *   npm run sherdog:outlook:find
 *   npm run sherdog:outlook:find -- --event "UFC Belgrade" \
...
function usage() {
  return `Usage:
  npm run sherdog:outlook:find
  npm run sherdog:outlook:find -- --event <name> --red <fighter> --blue <fighter>
...
async function run() {
  ...
  const match = await discoverSherdogOutlookPreview(target, {
    permissionScope: config.sherdog.permissionScope,
    baseUrl: config.sherdog.baseUrl,
  });
  if (match === undefined) {
    console.error(
      "Sherdog has not published the matching fight outlook in its news feed yet. Re-run this command closer to the event.",
    );
    process.exitCode = 2;
    return;
  }
